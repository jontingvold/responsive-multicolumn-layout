...
;(function(define){define(function(require,exports,module){
	...
	function FTColumnflow(target, viewport, userConfig) {
		...
		function _setFixedElementHeight(element) {
			...
			// Make figure images width 100% and keep the ratio
			if (element.nodeName == "FIGURE") {
				...
				for (var index = 0; index < imageElements.length; ++index) {
				    ...
				    var figureWidth = parseInt(element.style.width)
				    ...
				    if (image.hasAttribute("width")
    					...
    					var width = parseInt(image.attributes.width.value)
    					var height = parseInt(image.attributes.height.value)
    					var proportion = height/width
    					var newHeight = figureWidth * proportion

    					// Max height 100% of viewportHeight
    					if (newHeight + 100 > config.layoutDimensions.pageInnerHeight) {
    							element.style.height = config.layoutDimensions.pageInnerHeight + "px"
    							image.style.width = (config.layoutDimensions.pageInnerHeight - 100) / proportion
    							image.style.height = config.layoutDimensions.pageInnerHeight - 100
    					} else {
    							element.style.height = ""
    							image.style.width = parseInt(figureWidth) + "px"
